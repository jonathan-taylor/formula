...
#        b = [self.coefs[term] for term in self.terms]
    ...
    def __sub__(self, other):
        ...
        if not is_formula(other):
            raise ValueError('only Formula objects can be subtracted from ' +
                             'a Formula')

        poly = self.poly - other.poly
        terms = _terms_from_poly(poly)
        for j, _ in enumerate(terms):
            for i, t in self._poly_reverse_lookup.items():
                terms[j] = terms[j].subs(i, t)
            for i, t in other._poly_reverse_lookup.items():
                terms[j] = terms[j].subs(i, t)
    ...
    def poly(self):
        ...
        Create a polynomial from a Formula,
        that can be used to add, multiply, subtract
        Formula instances.

        The polynomial has variables given by
        ["v%d" % t for t in formula.terms]
        except for those t that are FactorTerm instances.
        These variables remain unchanged so that 
        their multiplication rules are used.
        """
        exp = self.mean
        id_terms = []
        self._poly_lookup = {}
        self._poly_reverse_lookup = {}
        for t in self.terms:
            if not is_factor_term(t):
                i = sympy.Symbol('v%d' % id(t))
            else:
                i = t
            self._poly_lookup[t] = i
            self._poly_reverse_lookup[i] = t
            id_terms.append(i)
            exp = exp.subs(t, i)
        return exp.as_poly(*np.unique(id_terms))
    ...
    def __mul__(self, other):
        ...
        terms = _terms_from_poly(poly)
        exp = 0
        for s in _terms_from_poly(self.poly):
            for t in _terms_from_poly(other.poly):
                exp += s * t
        terms = _terms_from_poly(exp.as_poly(*np.unique(spoly.args[2] + opoly.args[2])))
        for j, _ in enumerate(terms):
            for i, t in self._poly_reverse_lookup.items():
                terms[j] = terms[j].subs(i, t)
            for i, t in other._poly_reverse_lookup.items():
    ...
    def __eq__(self, other):
        s = self.unique.terms
        o = other.unique.terms
        if s.shape != o.shape:
            return False
        return np.alltrue(np.equal(s, o))
...
def _terms_from_poly(poly):
    """
    Given an polynomial involving variables
    ["v%d" % t for t in formula.terms]
    return the corresponding sympy expression
    with the variable 'v%d' % id(t)
    substited with t.

    This need not be a polynomial, but
    it will usually be, since it is
    called in Formula.__add__, Formula.__sub__
    and Formula.__mul__
    """
    poly_coef, poly_degrees, poly_vars, _ = poly.args

    terms = []
    for degree in poly_degrees:
        c = np.product(poly_coef)
        if c != 0:
            term = np.product([v**d for v,d in zip(poly_vars, degree)])
            terms.append(term)
    return terms
