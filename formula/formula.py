...
class CategoricalFormula(object):
    ...
    """
    Instantiated with a sequence or dictionary of [(expr, [factor])].
    """

    def __init__(self, *expr_factor_tuples):
        self.expr_factor_dict = dict(expr_factor_tuples)
        if "constant" in self.expr_factor_dict:
            self.expr_factor_dict[sympy.Number(1)] = list(self.expr_factor_dict['constant'])
            del(self.expr_factor_dict['constant'])
        for expr in self.expr_factor_dict:
            if not self.expr_factor_dict[expr]:
                self.expr_factor_dict[expr] = [()]
                

    def Rformula(self, add_intercepts=True):
        """
        ...
        if add_intercepts:
            expr_factor_dict.setdefault(sympy.Number(1), []).append(())
        ...
        for expr in expr_factor_dict:
            ...
            for subset_of_factors in expr_factor_dict[expr]:
                all_factors = all_factors.union(set(subset_of_factors))
                t = tuple(sorted([f.name for f in subset_of_factors]))
                subsets_of_names.setdefault(len(t), []).append(t)

            # in order to mimic R properly,
            # one needs to add a 1
            # for each expression
            ...
            if subsets_of_names:
                if all_factors:
                    factor_dict = dict([(f.name, Factor(f.name, sorted(f.levels)))
                                        for f in all_factors])

                    sorted_subsets_of_names = []
                    for i in sorted(subsets_of_names.keys()):
                        sorted_subsets_of_names += sorted(subsets_of_names[i])

                    codings = factor_codings(*[sorted(f) for f in sorted_subsets_of_names])
                    for prod_of_factors in codings:
                        cur_formula = I
                        for n, c in codings[prod_of_factors]:
                            if c == 'indicator':
                                cur_formula = cur_formula * factor_dict[n].formula
                            else:
                                cur_formula = cur_formula * factor_dict[n].drop_first
                        formula = formula + cur_formula
                else:
                    formula = Formula([1])
            final_formula = final_formula + formula * Formula([expr])
        return final_formula.delete_terms(Formula([0]))
